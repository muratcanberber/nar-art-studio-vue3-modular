...
export const useMaterials = defineStore('materials', {
    ...
    categories: [
      {
        id: 'canvas',
        name: 'Tuvaller',
        description: 'Hazir ve ozel olcu tuval stoklari',
        items: [
      ...
      },
      {
        id: 'frame',
        name: 'Cerceveler',
        description: 'Ahsap ve MDF cerceveler',
        items: [
          createItem('frame-black-18', 'MDF Cerceve Siyah 18mm', 6, 10, 4, 'Raf B3', 95),
          createItem('frame-oak-22', 'Masif Mese 22mm', 3, 5, 1, 'Raf B5', 165)
        ]
      },
      {
        id: 'relief',
        name: 'Rolyef Pasta',
        description: 'Doku calismalarinda kullanilan rolyef pastalar',
        items: [
          createItem('relief-standard', 'Akrelik Rolyef Pasta 500ml', 5, 4, 2, 'Raf C1', 120),
          createItem('relief-premium', 'Premium Rolyef Pasta 1L', 2, 3, 1, 'Raf C2', 210)
        ]
      }
    ]
  }),
  ...
  actions: {
    ...
    updateQuantity(categoryId: string, itemId: string, qty: number) {
      const category = this.categories.find(cat => cat.id === categoryId)
      const material = category?.items.find(item => item.id === itemId)
      if (!material) return
